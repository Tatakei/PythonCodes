...
if __name__ == "__main__":
    # Red text for prompt
    hex_color = input(
        "\033[31mEnter a hex color code (e.g., #FF5733): \033[0m")
    ...
    try:
        ...
        print(f"\033[33mThe HSV color is: Hue = {hsv_color[0]:.2f}°, Saturation = {
              hsv_color[1]:.2f}%, Value = {hsv_color[2]:.2f}%\033[0m")  # Blue text

        hsl_color = rgb_to_hsl(*rgb_color)
        print(f"\033[33mThe HSL color is: Hue = {hsl_color[0]:.2f}°, Saturation = {
              hsl_color[1]:.2f}%, Lightness = {hsl_color[2]:.2f}%\033[0m")  # Blue text

        cmy_color = rgb_to_cmy(*rgb_color)
        print(f"\033[34mThe CMY color is: Cyan = {cmy_color[0]:.2f}%, Magenta = {
              cmy_color[1]:.2f}%, Yellow = {cmy_color[2]:.2f}%\033[0m")  # Blue text

        cmyk_color = rgb_to_cmyk(*rgb_color)
        print(f"\033[32mThe CMYK color is: Cyan = {cmyk_color[0]:.2f}%, Magenta = {
              cmyk_color[1]:.2f}%, Yellow = {cmyk_color[2]:.2f}%, Black = {cmyk_color[3]:.2f}%\033[0m")  # Blue text

        xyz_color = rgb_to_xyz(*rgb_color)
        print(f"\033[34mThe XYZ color is: X = {xyz_color[0]:.2f}, Y = {
              xyz_color[1]:.2f}, Z = {xyz_color[2]:.2f}\033[0m")  # Blue text

        lab_color = rgb_to_lab(*rgb_color)
        print(f"\033[32mThe Lab color is: L = {lab_color[0]:.2f}, a = {
              lab_color[1]:.2f}, b = {lab_color[2]:.2f}\033[0m")  # Blue text

        lch_color = rgb_to_lch(*rgb_color)
        print(f"\033[34mThe LCH color is: L = {lch_color[0]:.2f}, C = {
              lch_color[1]:.2f}, H = {lch_color[2]:.2f}°\033[0m")  # Blue text

        yuv_color = rgb_to_yuv(*rgb_color)
        print(f"\033[32mThe YUV color is: Y = {yuv_color[0]:.2f}, U = {
              yuv_color[1]:.2f}, V = {yuv_color[2]:.2f}\033[0m")  # Blue text

        ycbcr_color = rgb_to_ycbcr(*rgb_color)
        print(f"\033[34mThe YCbCr color is: Y = {ycbcr_color[0]:.2f}, Cb = {
              ycbcr_color[1]:.2f}, Cr = {ycbcr_color[2]:.2f}\033[0m")  # Blue text

        hex_color = rgb_to_hex(*rgb_color)
        print(f"\033[32mThe HEX color code is: {
              hex_color}\033[0m")  # Blue text
        ...
        print(f"\033[32mThe CSS RGBA format (with alpha 1.0) is: {
              css_rgba}\033[0m")  # Blue text

        percentage = rgb_to_percentage(*rgb_color)
        print(f"\033[34mThe percentage values are: R = {percentage[0]:.2f}%, G = {
              percentage[1]:.2f}%, B = {percentage[2]:.2f}%\033[0m")  # Blue text

        grayscale = rgb_to_grayscale(*rgb_color)
        print(f"\033[32mThe Grayscale value is: {
              grayscale}\033[0m")  # Blue text

        complementary_color = get_complementary_color(*rgb_color)
        print(f"\033[34mThe Complementary color (RGB) is: {
              complementary_color}\033[0m")  # Blue text

        triadic_colors = get_triadic_colors(*rgb_color)
        print(f"\033[32mThe Triadic colors (RGB) are: {
              triadic_colors}\033[0m")  # Blue text

        tetradic_colors = get_tetradic_colors(*rgb_color)
        print(f"\033[34mThe Tetradic colors (RGB) are: {
              tetradic_colors}\033[0m")  # Blue text

    except ValueError:
        # Red text
        print(
            "\033[31mInvalid HEX color code! Please enter a valid HEX color code (e.g., #FF5733).\033[0m")
